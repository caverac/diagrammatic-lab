...
/** A permutation together with its length: a node of the Bruhat poset. */
export interface PosetNode {
  readonly perm: Permutation
  readonly length: number
}

/** The Bruhat order of S_n as a graded poset (no layout - that is the UI's job). */
export interface BruhatPoset {
  readonly nodes: readonly PosetNode[]
...
/**
 * Build the Bruhat order of S_n as a graded poset. Nodes are ranked by length;
 * a cover joins `u` to `w` exactly when `u <= w` and `w` has length one greater
 * (Bruhat order is graded by length, so these are the covering relations).
 */
export function bruhatPoset(n: number): BruhatPoset {
  const perms = symmetricGroup(n)
  const lengths = perms.map(permLength)
  const nodes = perms.map((perm, index) => ({ perm, length: lengths[index] }))
  ...
  return { nodes, covers, maxLength: (n * (n - 1)) / 2 }
}
